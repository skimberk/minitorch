...
@pytest.mark.task0_2
@given(floats(min_value=-100, max_value=36, allow_nan=False))
...
def test_transitive(a: float, b: float, c: float) -> None:
    ...
    if lt(a, b) == 1.0 and lt(b, c) == 1.0:
        assert lt(a, c) == 1.0
    if lt(b, c) == 1.0 and lt(c, a) == 1.0:
        assert lt(b, a) == 1.0
    if lt(c, a) == 1.0 and lt(a, b) == 1.0:
        assert lt(c, b) == 1.0
...
@pytest.mark.task0_2
@given(small_floats, small_floats)
def test_is_close(a: float, b: float) -> None:
    """
    Ensure that is_close behaves as expected
    """
    assert is_close(a, a) == 1.0
    assert is_close(b, b) == 1.0
    assert 0.01 == 1e-2
    if abs(a - b) < 0.01:
        assert is_close(a, b) == 1.0
    else:
        assert is_close(a, b) == 0.0
